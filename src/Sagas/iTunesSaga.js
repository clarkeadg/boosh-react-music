...
/* SCHEMAS */
import iTunesSchema from '../Schemas/iTunesSchema'
...
export default (api) => {
  ...
  function * worker (meta) {
    ...
    //console.log('GET ITUNES', meta)

    // make the call to the api
    const response = yield call(api.getTopMusicVideos, meta)

    //console.log('GOT ITUNES', response.data.feed.entry)

    // success?
    if (response && response.ok) {

      let payload = normalize(response.data.feed.entry, arrayOf(iTunesSchema));
     ...
     // yield put(Actions.getUsersSuccess(payload))
      yield put(Actions.getItunesSuccess(payload))

    } else {
      yield put(Actions.getItunesFailure())
    }
  }
  ...
  function * watcher () {
    //while (true) {
      const action = yield take(Types.GET_ITUNES_REQUEST)
      const { meta } = action
